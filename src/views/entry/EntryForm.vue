<template>
  <div class="entry-form">
    <el-card class="form-card">
      <template #header>
        <span>个人主申请录入</span>
        <el-button @click="goBack" text style="color: #fff; border-color: #fff;">返回列表</el-button>
      </template>
    ...
    <div class="form-actions">
      ...
      <el-button @click="saveForm">录入保存</el-button>
      <el-button @click="cancelEntry">录入取消</el-button>
      <el-button @click="showRemarkDialog = true">录入备注</el-button>
      <el-button @click="openOperationRecords">操作记录</el-button>
      <el-button type="primary" @click="submitForm">录入提交</el-button>
    </div>
    ...
    <el-dialog title="录入备注" v-model="showRemarkDialog" width="500px">
      <el-form>
...
import { ElMessage, ElMessageBox, ElDialog, ElDrawer } from 'element-plus'
import api from '@/api'
...
const cancelEntry = () => {
  ElMessageBox.prompt('请输入取消原因', '录入取消', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    inputPlaceholder: '请输入取消原因'
  }).then(({ value }) => {
    if (value && value.trim()) {
      ElMessage.success('录入取消操作成功')
      router.push('/entry/transfer')
    } else {
...
const fieldOptions = {
  ...
  customerNationality: [{ label: '中国', value: 'CN' }, { label: '美国', value: 'US' }, { label: '日本', value: 'JP' }, { label: '韩国', value: 'KR' }, { label: '英国', value: 'GB' }, { label: '法国', value: 'FR' }, { label: '德国', value: 'DE' }, { label: '意大利', value: 'IT' }, { label: '加拿大', value: 'CA' }, { label: '澳大利亚', value: 'AU' }, { label: '新加坡', value: 'SG' }, { label: '马来西亚', value: 'MY' }, { label: '泰国', value: 'TH' }, { label: '印度', value: 'IN' }, { label: '俄罗斯', value: 'RU' }, { label: '西班牙', value: 'ES' }, { label: '葡萄牙', value: 'PT' }, { label: '荷兰', value: 'NL' }, { label: '瑞士', value: 'CH' }, { label: '瑞典', value: 'SE' }, { label: '挪威', value: 'NO' }, { label: '丹麦', value: 'DK' }, { label: '芬兰', value: 'FI' }, { label: '波兰', value: 'PL' }, { label: '奥地利', value: 'AT' }, { label: '比利时', value: 'BE' }, { label: '希腊', value: 'GR' }, { label: '爱尔兰', value: 'IE' }, { label: '卢森堡', value: 'LU' }, { label: '捷克', value: 'CZ' }, { label: '匈牙利', value: 'HU' }, { label: '斯洛伐克', value: 'SK' }, { label: '斯洛文尼亚', value: 'SI' }, { label: '克罗地亚', value: 'HR' }, { label: '保加利亚', value: 'BG' }, { label: '罗马尼亚', value: 'RO' }, { label: '乌克兰', value: 'UA' }, { label: '白俄罗斯', value: 'BY' }, { label: '土耳其', value: 'TR' }, { label: '埃及', value: 'EG' }, { label: '南非', value: 'ZA' }, { label: '尼日利亚', value: 'NG' }, { label: '肯尼亚', value: 'KE' }, { label: '巴西', value: 'BR' }, { label: '阿根廷', value: 'AR' }, { label: '墨西哥', value: 'MX' }, { label: '智利', value: 'CL' }, { label: '哥伦比亚', value: 'CO' }, { label: '秘鲁', value: 'PE' }, { label: '委内瑞拉', value: 'VE' }, { label: '印度尼西亚', value: 'ID' }, { label: '菲律宾', value: 'PH' }, { label: '越南', value: 'VN' }, { label: '柬埔寨', value: 'KH' }, { label: '老挝', value: 'LA' }, { label: '缅甸', value: 'MM' }, { label: '孟加拉国', value: 'BD' }, { label: '巴基斯坦', value: 'PK' }, { label: '尼泊尔', value: 'NP' }, { label: '斯里兰卡', value: 'LK' }, { label: '伊朗', value: 'IR' }, { label: '沙特阿拉伯', value: 'SA' }, { label: '阿联酋', value: 'AE' }, { label: '以色列', value: 'IL' }, { label: '科威特', value: 'KW' }, { label: '卡塔尔', value: 'QA' }, { label: '巴林', value: 'BH' }, { label: '阿曼', value: 'OM' }, { label: '约旦', value: 'JO' }, { label: '黎巴嫩', value: 'LB' }, { label: '叙利亚', value: 'SY' }, { label: '伊拉克', value: 'IQ' }, { label: '阿富汗', value: 'AF' }, { label: '新西兰', value: 'NZ' }, { label: '冰岛', value: 'IS' }, { label: '马尔代夫', value: 'MV' }, { label: '毛里求斯', value: 'MU' }, { label: '塞舌尔', value: 'SC' }, { label: '斐济', value: 'FJ' }, { label: '巴布亚新几内亚', value: 'PG' }, { label: '萨摩亚', value: 'WS' }, { label: '汤加', value: 'TO' }, { label: '瑙鲁', value: 'NR' }, { label: '图瓦卢', value: 'TV' }, { label: '基里巴斯', value: 'KI' }, { label: '马绍尔群岛', value: 'MH' }, { label: '帕劳', value: 'PW' }, { label: '密克罗尼西亚', value: 'FM' }, { label: '所罗门群岛', value: 'SB' }, { label: '瓦努阿图', value: 'VU' }, { label: '新喀里多尼亚', value: 'NC' }, { label: '法属波利尼西亚', value: 'PF' }, { label: '关岛', value: 'GU' }, { label: '美属萨摩亚', value: 'AS' }, { label: '波多黎各', value: 'PR' }, { label: '巴哈马', value: 'BS' }, { label: '古巴', value: 'CU' }, { label: '多米尼加', value: 'DO' }, { label: '海地', value: 'HT' }, { label: '牙买加', value: 'JM' }, { label: '特立尼达和多巴哥', value: 'TT' }, { label: '巴巴多斯', value: 'BB' }, { label: '格林纳达', value: 'GD' }, { label: '圣卢西亚', value: 'LC' }, { label: '圣文森特和格林纳丁斯', value: 'VC' }, { label: '安提瓜和巴布达', value: 'AG' }, { label: '多米尼克', value: 'DM' }, { label: '圣基茨和尼维斯', value: 'KN' }, { label: '伯利兹', value: 'BZ' }, { label: '危地马拉', value: 'GT' }, { label: '洪都拉斯', value: 'HN' }, { label: '萨尔瓦多', value: 'SV' }, { label: '尼加拉瓜', value: 'NI' }, { label: '哥斯达黎加', value: 'CR' }, { label: '巴拿马', value: 'PA' }, { label: '格陵兰', value: 'GL' }, { label: '百慕大', value: 'BM' }, { label: '开曼群岛', value: 'KY' }, { label: '特克斯和凯科斯群岛', value: 'TC' }, { label: '英属维尔京群岛', value: 'VG' }, { label: '美属维尔京群岛', value: 'VI' }, { label: '安圭拉', value: 'AI' }, { label: '蒙特塞拉特', value: 'MS' }, { label: '圭亚那', value: 'GY' }, { label: '苏里南', value: 'SR' }, { label: '法属圭亚那', value: 'GF' }, { label: '厄瓜多尔', value: 'EC' }, { label: '玻利维亚', value: 'BO' }, { label: '巴拉圭', value: 'PY' }, { label: '乌拉圭', value: 'UY' }, { label: '马尔维纳斯群岛', value: 'FK' }, { label: '莱索托', value: 'LS' }, { label: '斯威士兰', value: 'SZ' }, { label: '纳米比亚', value: 'NA' }, { label: '博茨瓦纳', value: 'BW' }, { label: '津巴布韦', value: 'ZW' }, { label: '赞比亚', value: 'ZM' }, { label: '马拉维', value: 'MW' }, { label: '莫桑比克', value: 'MZ' }],
  ...
  customerCompanyOccupation: [{ label: '管理人员', value: '01' }, { label: '专业技术', value: '02' }, { label: '普通职员', value: '03' }],
...
const saveForm = async () => {
  ...
  ElMessage.success('录入保存成功')
}

const submitForm = async () => {
  const result = await validateAllForms()
  if (!result.valid) {
    ElMessage.error('数据校验不通过，无法提交，请检查红色提示信息')
    if (result.errors.length > 0) {
      result.errors.forEach(error => {
        ElMessage.warning(error)
      })
...
onMounted(() => {
  ...
  if (params) {
    ...
    formData.applicationId = params.applicationId || '202607020000000001'
...
:deep(.el-date-picker__editor:focus) {
  border-color: #c0392b;
}
