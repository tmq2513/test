...
function formatNumToSymbol(pinyin2) {
  let suffixR = "";
  if (pinyin2.length > 2 && pinyin2.endsWith("r")) {
    const secondToLast = Number(pinyin2[pinyin2.length - 2]);
    if (secondToLast >= 0 && secondToLast <= 4) {
      suffixR = "r";
      pinyin2 = pinyin2.slice(0, -1);
    }
  }
  ...
  if (lastChar >= 0 && lastChar <= 4) {
    for (let key in toneMap) {
      if (pinyin2.includes(key)) {
        return pinyin2.slice(0, pinyin2.length - 1).replace(key, toneMap[key][lastChar]) + suffixR;
      }
    }
    return pinyin2 + suffixR;
  } else {
    return pinyin2 + suffixR;
  }
}
function formatSymbolToNum(pinyin2) {
  if (pinyin2.endsWith("r") && pinyin2.length > 1 && !/^[eēéěè]r$/.test(pinyin2)) {
    const pinyinWithoutR = pinyin2.slice(0, -1);
